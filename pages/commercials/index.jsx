import Head from "next/head";
import { client } from "@/sanity/lib/client";
import { groq } from "next-sanity";
import { motion } from "framer-motion";
import Layout from "@/_components/layout";
import PageHeading from "@/_components/page-heading";
import VideoSlot from "@/_components/video-slot";
import AnimatedText from "@/_components/animated-text";

const regex =
  /(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/;

export async function getStaticProps() {
  const data =
    await client.fetch(groq`*[_type == "commercial"] | order(orderRank){
    ...,
    "videoPlaceholder": videoPlaceholder.asset->url,
  }`);

  return {
    props: {
      data,
    },
  };
}

export default function Commercials({ data }) {
  return (
    <>
      <Head>
        <title>Nati Makeupik &#x2022; Commercials</title>
      </Head>
      <Layout>
        <section className="relative px-4 pb-24 2xl:mx-auto 2xl:max-w-screen-2xl 2xl:px-0">
          <PageHeading>Commercials</PageHeading>

          <div className="relative mx-auto flex max-w-screen-lg flex-col items-center justify-center gap-24 md:px-10">
            {data.map((project) => {
              return (
                <div
                  key={project._id}
                  className="w-full max-w-screen-lg overflow-hidden"
                >
                  <div className="relative isolate min-h-[190px] overflow-hidden rounded-[36px] shadow-red/5 lg:min-h-[400px]">
                    <VideoSlot project={project} />
                  </div>
                  <div className="max-w-[90%] pt-6">
                    <motion.h2
                      initial={{ opacity: 0 }}
                      whileInView={{ opacity: 1 }}
                      viewport={{ margin: "-10%", once: true }}
                      transition={{
                        type: "tween",
                        ease: "easeInOut",
                        duration: 0.25,
                      }}
                      className="relative flex-1 pb-1 text-3xl tracking-tight text-red lg:text-5xl lg:leading-snug"
                    >
                      {project.client}
                    </motion.h2>
                  </div>
                </div>
              );
            })}
          </div>
          <div className="mx-auto max-w-screen-lg px-4 pb-24 pt-36 text-center text-4xl leading-normal md:px-10 lg:text-[42px] lg:leading-normal xl:text-6xl xl:leading-tight">
            <AnimatedText>
              <h4 className="mx-auto w-full max-w-2xl text-pretty lg:max-w-3xl">
                Assisted in commercials of Play, VISA, Santander, Google,
                Biedronka, Fanta, Media Expert, Eurosport, Huawei, BMW, Redbull,
                H&M, Lewiatan and many more.
              </h4>
            </AnimatedText>
          </div>
        </section>
      </Layout>
    </>
  );
}
